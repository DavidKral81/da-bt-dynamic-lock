"""Controls drawn by hand - for the app AND the installer.
...
the installer used to keep Tk's default marks while the app had hand-drawn
ones, and the two looked nothing alike. The language flags are here for the
same reason.

Usage:
    canvas = tk.Canvas(parent, width=marks.SIZE, height=marks.SIZE, ...)
    marks.draw(canvas, on=True)
    marks.LanguageFlags(header, switch_language, "#e6ebf2").pack(side="right")
"""
...
class LanguageFlags(tk.Frame):
    """The language switch: one small flag per language, top right corner.
    ...
    `action` is called with the language CODE - never with the displayed
    name, which is text for the eye and not a data value.
    """
